...
impl BillPayments {
    ...
    /// @notice Restore one archived bill back into active storage.
    /// @dev Only the original bill owner can restore. The restored bill remains marked as paid to
    /// prevent duplicate payment execution.
    /// @param caller Authenticated owner of the archived bill.
    /// @param bill_id Archived bill identifier.
    /// @return Ok(()) on successful restoration.
    /// @security Enforces strict owner check before state mutation.
